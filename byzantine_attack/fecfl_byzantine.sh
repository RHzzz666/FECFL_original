#!/bin/bash
# 使用FGSM对抗攻击（指定0-30号客户端，中等强度）
for seed in 66
do
    dir='../save_results/byzantine/fgsm_specific/fecfl/cifar10'
    if [ ! -e $dir ]; then
    mkdir -p $dir
    fi
    current_time=$(date +"%Y%m%d-%H%M%S")
    current_time_safe=$(echo "$current_time" | tr ':' '_')
    python ./main_fecfl_byzantine.py --trial=1 \
    --rounds=50 \
    --num_users=100 \
    --frac=1 \
    --local_ep=10 \
...
# for seed in 66
# do
#     dir='../save_results/byzantine/noise_specific/fecfl/cifar10'
#     if [ ! -e $dir ]; then
#     mkdir -p $dir
#     fi
#     current_time=$(date +"%Y%m%d-%H%M%S")
...
# for seed in 66
# do
#     dir='../save_results/byzantine/blur_specific/fecfl/cifar10'
#     if [ ! -e $dir ]; then
#     mkdir -p $dir
#     fi
#     current_time=$(date +"%Y%m%d-%H%M%S")
