...
class Client_FedProx(object):
    def __init__(self, name, model, local_bs, local_ep, lr, momentum, device, 
                 train_dl_local = None, test_dl_local = None, mu=0.001):
    ...
    def train(self, is_print = False):
        ...
        global_weight_collector = [param.data.clone().detach() for param in self.net.parameters()]
    ...
    def train_unsupervised(self, epoch=0):
        ...
        for iteration in range(epoch):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                optimizer.zero_grad()
                log_probs = self.net(images)
                loss = self.loss_func(log_probs, images)
