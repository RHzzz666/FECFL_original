...
class OriginalCINIC10(FedDataset):
    """Label change for CINIC10 and patrition them.
    ...
    def __init__(self, root, save_dir, num_clients, partition_method="noniid", alpha=0.5):
        ...
        self.alpha = alpha
        # "./datasets/labels_changed_cinic10/"
    ...
    def preprocess(self, labels_mapping=None):
        print("########### Preprocessing original CINIC10 ###########")
        ...
        if self.partition_method > "noniid-#label0" and self.partition_method <= "noniid-#label9":
            ...
            partitions = noniid_slicing_label_skew(cinic10_full, self.num, 10, num_label)
        elif self.partition_method == "noniid-labeldir":
            partitions = noniid_slicing_with_dirichlet(cinic10_full, self.num, 10, self.alpha)
        elif self.partition_method == "pathological":
            partitions = pathological_slicing(cinic10_full, self.num, 10, 2)
        elif self.partition_method == "homo":
            partitions = random_slicing(cinic10_full, self.num)
        else:
