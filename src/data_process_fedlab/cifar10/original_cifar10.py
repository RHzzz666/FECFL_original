...
class OriginalCIFAR10(FedDataset):
    ...
    def preprocess(self, labels_mapping=None):
        print("########### Preprocessing original CIFAR10 ###########")
        ...
        cifar10_full = ConcatCIFAR10(cifar10_train, cifar10_test)

        if self.partition_method > "noniid-#label0" and self.partition_method <= "noniid-#label9":
            num_label = eval(self.partition_method[13:])
            partitions = noniid_slicing_label_skew(cifar10_full, self.num, 10, num_label)
        elif self.partition_method == "noniid-#dirichlet":
            partitions = noniid_slicing_with_dirichlet(cifar10_full, self.num, 10, self.alpha)
        elif self.partition_method == "pathological":
            partitions = pathological_slicing(cifar10_full, self.num, 10, 2)
        elif self.partition_method == "pathological#2":
            groups = [[0, 8], [1, 9], [2, 4], [3, 5], [6, 7]]
            partitions = pathological_slicing_2(cifar10_full, self.num, groups)
