...
from src.client.client_fedsoft import Client_FedSoft
...
for idx in range(args.num_users):
    ...
    clients.append(Client_FedSoft(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep,
                                args.lr, args.momentum, args.device, train_ds_local, test_ds_local))

'''fedsoft'''

w_glob_per_cluster = [copy.deepcopy(initial_state_dict) for _ in range(args.nclusters)]


def get_importance_weights_matrix(clients, cluster_models):
    importance_weights_matrix = []
    for client in clients:
        importance_weights_matrix.append(client.estimate_importance_weights(args.nclusters, cluster_models))
    ...
    return importance_weights_matrix
...
cnt = args.num_users
selection_size = max(int(args.frac * args.num_users), 1) // args.nclusters
for r in range(1):
    print(f'Round {r}')
    ...
    importance_weights_matrix = get_importance_weights_matrix(clients, w_glob_per_cluster)

    distance_matrix = 1 - importance_weights_matrix
    print('')
    ...
    selection = []
    for s in range(args.nclusters):
        selection.append(np.random.choice(a=range(args.num_users), size=selection_size,
...
for iteration in range(args.rounds):
    ...
    if iteration % args.estimation_interval == 0:
        importance_weights_matrix = get_importance_weights_matrix(clients, w_glob_per_cluster)
        distance_matrix = 1 - importance_weights_matrix
        print('')
        print('Distance Matrix')
        ...
        clusters = selection
        print('')
        print(f'Clusters at round {iteration}: ')
        print(clusters)
        clients_clust_id = {i: None for i in range(args.num_users)}
        for i in range(args.num_users):
            for j in range(len(clusters)):
    ...
    idxs_users = np.array(clusters).flatten()
